...
const types = readFileSync('svg.filter.js.d.ts', 'utf8')
let emitted = false
...
const distExtras = {
  ...
  generateBundle() {
    ...
    this.emitFile({
      type: 'asset',
      fileName: 'svg.filter.d.mts',
      source: types
    })
    this.emitFile({
      type: 'asset',
      fileName: 'svg.filter.d.ts',
      source: types
    })
  }
}
